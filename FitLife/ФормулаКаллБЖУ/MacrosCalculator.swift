...
struct MacrosCalculator {
    ...
    static func calculateCaloriesMifflin(
        ...
        return await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                switch goal {
                case .loseWeight:
                    result = Int(maintenanceCalories * 0.8)
                    // Снижение веса: уменьшаем калории на 20%.
                case .currentWeight:
                    result = Int(maintenanceCalories)
                    // Сохранение веса: без изменений.
                case .gainWeight:
                    result = Int(maintenanceCalories * 1.2)
    ...
    static func calculateMacros(
        calories: Int,
        goal: WeightGoal
    ) async -> (proteins: Int, fats: Int, carbs: Int) {
        // Метод принимает:
        ...
        return await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                continuation.resume(returning: (            proteins: proteinCalories / 4,
